...
describe('Add-ons form section', () => {
  ...
  it('should be able to move foward and go back and see same options selected', () => {
    ...
    cy.get('[id="onlineServiceInput"]').find('input').should('be.checked')
    cy.get('[id="largerStorageInput"]').find('input').should('not.be.checked')
    cy.get('[id="customProfileInput"]').find('input').should('not.be.checked')
  })
})
